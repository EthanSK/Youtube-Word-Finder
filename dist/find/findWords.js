...
async function findWords() {
    wordFoundCounts = [...Array(userDefaults_1.userDefaultsOnStart.words.length).fill(0)];
    for (let i = 0; i < userDefaults_1.userDefaultsOnStart.maxNumberOfVideos; i++) {
        const id = await getVideoMetadata_1.default(i);
        const videoMetadata = await processVideoMetadata_1.default(id);
...
function searchWordsInSubs(videoMetadata) {
    ...
    for (let i = 0; i < userDefaults_1.userDefaultsOnStart.words.length; i++) {
        ...
        if (wordFoundCounts[i] &&
            wordFoundCounts[i] >= userDefaults_1.userDefaultsOnStart.numberOfWordReps)
            continue;
        for (const phrase of videoMetadata.subtitles.phrases) {
            ...
            if (videoMetadata.subtitles.isIndividualWords) {
                if (word.mainWord === words_1.filterWord(phrase.text)) {
                    result.push(clip);
                    wordFoundCounts[i] += 1;
                }
            }
                ...
                for (const subPhrase of phrase.text.split(/\s+/)) {
                    if (word.mainWord === words_1.filterWord(subPhrase)) {
                        result.push(clip);
                        wordFoundCounts[i] += 1;
