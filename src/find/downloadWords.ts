  ...
  return new Promise((resolve, reject) => {
    ...
    proc.stderr.on("data", function (data: string) {
      ...
      if (data.includes("HTTP error 403 Forbidden")) {
        ...
        // )
        sendToConsoleOutput(
          "Error downloading video: raw video URL expired. Need to get updated metadata for video. If this problem persists, delete the temp folder in your chosen output location.",
          "error"
        )
      ...
      if (data.includes("error") || data.includes("Error")) {
        ...
        sendToConsoleOutput(
          `Error downloading video ${clip.id}: ${data}. Execution will continue.`,
          "error"
        )
      }
    })
